...
import { Users, FileCheck, Clock, CheckCircle, XCircle, BarChart3, Shield, Search } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [statsData, applicationsData] = await Promise.all([
        ...
        AdminService.getAllApplications(currentPage, 20, selectedStatus || undefined)
      ]);
  ...
  const handleReviewApplication = async (applicationId: string, action: 'approve' | 'reject', notes?: string) => {
    try {
      await AdminService.reviewApplication(applicationId, action, user!.id, notes);
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {applications.map((application) => (
                  <tr key={application.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <div className="text-sm text-gray-500">
                          Step {application.current_step}/5
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getRiskColor(application.risk_level)}`}>
